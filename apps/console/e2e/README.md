...
> **Status:** active. Storage-state auth and multi-role seeding are implemented.
...
apps/console/e2e/
├── README.md            # this file
├── tests/               # Playwright spec files, one per area
...
`console-e2e` runs the real suite on every PR. Notes if you touch it:
...
See `fixtures/auth.ts` and `global-setup.ts` for the implementation.

## Conventions

- **One spec file per product area.** For example

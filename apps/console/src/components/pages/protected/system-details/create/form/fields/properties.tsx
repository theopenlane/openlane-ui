  ...
  const renderTags = () => {
    ...
    return (
      <div className={`${isEditAllowed ? 'cursor-pointer' : 'cursor-not-allowed'} flex flex-wrap gap-2`}>
        {hasTags ? data?.tags?.map((tag, index) => tag && <TagChip tag={tag} key={index} />) : <span className="text-muted-foreground text-sm italic">No tags</span>}
      </div>
    )
  }
...
"
        ...
        {isEditing || internalEditing === 'tags' ? (
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <div className="w-62.5" ref={triggerRef}>
                <MultipleSelector
                  options={tagOptions}
                  hideClearAllButton
                  placeholder="Add tag..."

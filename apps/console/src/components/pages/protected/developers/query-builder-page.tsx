...
type EntityField = { kind: 'scalar'; name: string; scalarKind: ScalarKind } | { kind: 'object'; name: string; subFields: { name: string; scalarKind: ScalarKind }[] }
...
export function QueryBuilderPage() {
  ...
  const entities = useMemo(() => (schema ? buildEntities(schema) : []), [schema])
  ...
  const selectedEntity = useMemo(() => entities.find((e) => e.queryName === selectedEntityName) ?? null, [entities, selectedEntityName])
  const scalarFields = useMemo(() => (selectedEntity?.fields.filter((f) => f.kind === 'scalar') ?? []) as Extract<EntityField, { kind: 'scalar' }>[], [selectedEntity])
  const objectFields = useMemo(() => (selectedEntity?.fields.filter((f) => f.kind === 'object') ?? []) as Extract<EntityField, { kind: 'object' }>[], [selectedEntity])

  // matches a field against a search term using both the raw and display name
  const matchesSearch = (name: string, term: string) => {
    const q = term.trim().toLowerCase()
    if (!q) return true
    return name.toLowerCase().includes(q) || toDisplayName(name).toLowerCase().includes(q)
  }

  const visibleScalarFields = scalarFields.filter((f) => matchesSearch(f.name, columnSearch))

  // an object stays visible when either its own name or one of its sub fields matches
  const visibleObjectFields = objectFields
    .map((f) => {
      const objectMatches = matchesSearch(f.name, relatedSearch)
      const subFields = objectMatches ? f.subFields : f.subFields.filter((sf) => matchesSearch(sf.name, relatedSearch))
      return { field: f, subFields }
    })
    .filter((o) => o.subFields.length > 0)
  ...
  const addCondition = () => {
    ...
    if (!first) return
    setConditions((prev) => [...prev, { id: crypto.randomUUID(), field: first.name, operator: operatorsForKind(first.scalarKind)[0], value: '' }])
  }
      ...
      <aside className="w-72 shrink-0 flex flex-col gap-4 overflow-y-auto">
          ...
          <>
            <div className="bg-card rounded-lg border border-border p-4">
              <div className="flex items-center justify-between mb-1">
                ...
                <button
                  onClick={() => {
                    const scalarNames = visibleScalarFields.map((f) => f.name)
                    const allChecked = scalarNames.length > 0 && scalarNames.every((n) => selectedFields.has(n))
                    setSelectedFields((prev) => {
                      const next = new Set(prev)
                      if (allChecked) {
                ...
                >
                  {visibleScalarFields.length > 0 && visibleScalarFields.every((f) => selectedFields.has(f.name)) ? 'Deselect all' : 'Select all'}
                </button>
              </div>
              ...
              <input
                ...
                className="w-full mb-2 rounded-md border border-border bg-background text-foreground text-xs px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-ring"
              />
              <div className="flex flex-col gap-1.5 max-h-64 overflow-y-auto">
                {visibleScalarFields.length === 0 ? (
                  ...
                  visibleScalarFields.map((f) => (
                    <label key={f.name} className="flex items-center gap-2 cursor-pointer group">
                      <input type="checkbox" checked={selectedFields.has(f.name)} onChange={() => toggleField(f.name)} className="rounded border-border accent-primary" />
            ...
            {selectedEntity.fields.some((f) => f.kind === 'object') && (
              <div className="bg-card rounded-lg border border-border p-4">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    onClick={() => {
                      const allKeys = visibleObjectFields.flatMap((o) => o.subFields.map((sf) => `${o.field.name}.${sf.name}`))
                      const allChecked = allKeys.length > 0 && allKeys.every((k) => selectedFields.has(k))
                      setSelectedFields((prev) => {
                        const next = new Set(prev)
                        if (allChecked) {
                          ...
                          setExpandedObjects(new Set(visibleObjectFields.map((o) => o.field.name)))
                    ...
                    className="text-xs text-primary hover:text-primary/80"
                  >
                    {visibleObjectFields.length > 0 && visibleObjectFields.flatMap((o) => o.subFields.map((sf) => `${o.field.name}.${sf.name}`)).every((k) => selectedFields.has(k))
                      ? 'Deselect all'
                      : 'Select all'}
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-2 max-h-64 overflow-y-auto">
                  {visibleObjectFields.length === 0 ? (
                    <p className="text-xs text-muted-foreground">No matching fields</p>
                  ) : (
                    visibleObjectFields.map(({ field: f, subFields }) => {
                      const subKeys = subFields.map((sf) => `${f.name}.${sf.name}`)
                      const allSelected = subKeys.every((k) => selectedFields.has(k))
                      const someSelected = subKeys.some((k) => selectedFields.has(k))
                      // auto expand while searching so matching sub fields are visible
                      const isExpanded = expandedObjects.has(f.name) || relatedSearch.trim().length > 0
                      return (
                        <div key={f.name}>
                          <div className="flex items-center gap-2">
                            <IndeterminateCheckbox checked={allSelected} indeterminate={someSelected && !allSelected} onChange={() => toggleObjectField(f.name, subFields)} />
                            <button onClick={() => toggleObjectExpand(f.name)} className="flex items-center gap-1 text-sm text-foreground hover:text-foreground/80 flex-1 text-left">
        ...
        {selectedEntity && (
          <div className="bg-card rounded-lg border border-border p-4">
              ...
              {conditions.map((cond, idx) => {
                ...
                return (
                  <div key={cond.id} className="flex flex-col gap-1.5 p-2 rounded-md bg-muted/30 border border-border/50">
                    {idx > 0 && <span className="text-xs text-muted-foreground font-medium">{combinator === 'and' ? 'AND' : 'OR'}</span>}
                    <div className="flex gap-1 items-center">
                      ...
                      <button onClick={() => removeCondition(cond.id)} className="shrink-0 text-muted-foreground hover:text-destructive text-xs px-1" aria-label="Remove filter">
                        ×
                      </button>
                    </div>
                    <select
            ...
            <button onClick={addCondition} className="mt-2 w-full text-xs text-primary hover:text-primary/80 border border-dashed border-border rounded-md py-1.5 hover:bg-muted/30 transition-colors">
              + Add filter
            </button>
          </div>
        )}
      ...
      <main className="flex-1 min-w-0 flex flex-col min-h-0">
        <div className="bg-card rounded-lg border border-border flex-1 min-h-0 flex flex-col">
          <div className="flex items-center border-b border-border px-4">
            ...
            {activeTab === 'results' && queryResult && (
              <div className="ml-auto flex items-center gap-2">
                ...
                <button onClick={exportCsv} className="text-xs rounded-md border border-border bg-background text-foreground px-3 py-1.5 hover:bg-muted transition-colors">
                  Export CSV
                </button>
              </div>
            )}
          ...
          <div className="flex-1 min-h-0 overflow-auto p-4">
            {activeTab === 'query' ? (
              <pre className="text-xs text-muted-foreground font-mono whitespace-pre-wrap">{generatedQuery || 'No query generated yet'}</pre>
            ) : queryError ? (
                            ...
                            <tr key={i} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                              {resultColumns.map((col) => (
                                <td key={col} className="text-xs text-foreground py-2 px-3 max-w-xs truncate">
                                  {flat[col] === null || flat[col] === undefined ? <span className="text-muted-foreground italic">—</span> : String(flat[col])}
                                </td>
                              ))}
                            </tr>
                          )
              ...
              <div className="flex flex-col items-center justify-center h-48 text-center gap-2">
                ...
                <p className="text-muted-foreground text-sm max-w-xs">Choose what data to report on, select the columns you want to see, optionally add filters, then click Run Report.</p>
              </div>
            )}
          </div>
        </div>

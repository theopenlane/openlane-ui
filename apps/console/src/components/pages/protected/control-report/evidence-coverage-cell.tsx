  ...
  return (
    <div className="flex flex-col gap-1 min-w-0">
      <div className="h-1.5 rounded-full bg-border overflow-hidden">
        <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, backgroundColor: worstStatus ? EvidenceStatusColors[worstStatus] : undefined }} />
      </div>
          ...
          <TooltipProvider delayDuration={300}>
            <Tooltip>
              <TooltipTrigger asChild>
                <EvidenceStatusBadge status={worstStatus} className="shrink-0 cursor-default" />
              </TooltipTrigger>
                  ...
                  <div className="text-xs min-w-[180px] max-w-[280px] space-y-1.5">
                    ...
                    {sortedCounts.map((entry) => (
                      <div key={entry.status} className="flex items-center justify-between gap-2">
                        <EvidenceStatusBadge status={entry.status} className="text-[10px] px-1.5 shrink-0" />

...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from '@theopenlane/ui/dialog'
import { FormField, FormItem, FormLabel, FormControl, Form } from '@theopenlane/ui/form'
...
import { useGetIdentityHolderAssociations } from '@/lib/graphql-hooks/identity-holder'
...
const MarkAsEvidenceDialog: React.FC<MarkAsEvidenceDialogProps> = ({ fileId, fileName, personnelId, onClose }) => {
  ...
  const { data: associationsData, isLoading: controlsLoading } = useGetIdentityHolderAssociations(personnelId)

  const personnelControls = useMemo(() => {
    return associationsData?.identityHolder?.controls?.edges?.map((edge) => edge?.node).filter((node): node is NonNullable<typeof node> => node != null) ?? []
  }, [associationsData])

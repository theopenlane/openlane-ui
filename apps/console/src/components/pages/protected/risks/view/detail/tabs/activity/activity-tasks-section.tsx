...
import { getActivityTaskColumns, getActivityTaskFilterFields, type ActivityTaskRow } from '../../../../../../../shared/crud-base/tabs/activity-tasks-config'
...
const ActivityTasksSection: React.FC<ActivityTasksSectionProps> = ({ riskId }) => {
  const router = useRouter()
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const { data: session } = useSession()
  const associationFilter = useMemo(() => {
    return riskId ? { hasRisksWith: [{ id: riskId }] } : {}
  }, [riskId])
  ...
  if (!hasSearchOrFilters && (tasks?.length ?? 0) === 0) {
    return <EmptyTabState description="Use tasks to track work related to this risk. Add a task to get started." />
  }

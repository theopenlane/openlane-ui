    ...
    <AccordionItem value={node.id}>
      <div className="flex justify-between items-center my-2">
        ...
        <div className="flex gap-2">
          {isEditAllowed ? (
            node.verified ? (
              <Button variant="outline" className="h-8 !px-2" icon={<Check />} iconPosition="left" disabled>
                Verified
              </Button>
            ) : (
              <Button variant="outline" className="h-8 !px-2" icon={<Check />} iconPosition="left" onClick={() => onMarkVerified(node.id)} disabled={permLoading || isUpdating}>
                Mark verified
              </Button>
            )
          ) : node.verified ? (
          ...
          {isEditAllowed && (
            <Button variant="outline" className="h-8 !px-2" icon={<Pencil />} iconPosition="left" onClick={() => onEdit(node)} disabled={permLoading || isUpdating}>
              Edit
            </Button>
          )}
        </div>

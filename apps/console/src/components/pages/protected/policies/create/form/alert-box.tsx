...
type THelperProps = {
  ...
  /** kick off AI generation for this policy name on mount, e.g. from a deep link */
  autoGenerateName?: string
}

const HelperText = ({ name, editorRef, onNameChange, autoGenerateName }: THelperProps) => {
  ...
  // deep-linked "create with AI": start generating as soon as the form loads
  const autoGeneratedRef = useRef(false)
  useEffect(() => {
    if (!autoGenerateName || !aiEnabled || autoGeneratedRef.current) return
    autoGeneratedRef.current = true
    generatePolicy(autoGenerateName, '')
    // generatePolicy is recreated each render; the ref guard makes this run once

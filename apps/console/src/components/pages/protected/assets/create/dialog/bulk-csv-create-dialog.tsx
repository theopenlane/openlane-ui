...
import { useCreateBulkCSVAsset } from '@/lib/graphql-hooks/assets'
...
const BulkCSVCreateDialog: React.FC<BulkCsvCreateDialogProps> = ({ trigger }) => {
  const [isOpen, setIsOpen] = useState<boolean>(false)
  const [uploadedFile, setUploadedFile] = useState<TUploadedFile | null>(null)
  const { successNotification, errorNotification } = useNotification()
  const { mutateAsync: createBulk, isPending: isSubmitting } = useCreateBulkCSVAsset()
  ...
  const handleFileUpload = async () => {
    ...
    try {
      await createBulk({ input: uploadedFile.file! })
      successNotification({
        title: 'Assets Created',
        description: `Assets have been successfully created`,
      })
      setIsOpen(false)
    } catch (error) {
  ...
  const handleCSVExport = async () => {
    await exportCSV({ filename: 'asset' })
  }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[640px] bg-secondary">
        ...
        <Callout title="CSV Format">
          <p className="text-sm">
            You can upload a csv containing Assets. Please refer to our{' '}
            <a href={`${GRAPHQL_OBJECT_DOCS}#asset`} target="_blank" className="text-brand hover:underline" rel="noreferrer">
              documentation
            </a>{' '}

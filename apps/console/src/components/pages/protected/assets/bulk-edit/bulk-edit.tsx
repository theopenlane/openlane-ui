...
import { useBulkEditAsset } from '@/lib/graphql-hooks/assets'
import {
  BulkEditDialogFormValues,
  BulkEditAssetsDialogProps,
  defaultObject,
  getAllSelectOptionsForBulkEditAssets,
  getMappedClearValue,
  InputType,
...
const fieldItemSchema = z.object({
  value: z.nativeEnum(SelectOptionBulkEditAssets).optional(),
  selectedObject: z
    .object({
      selectOptionEnum: z.nativeEnum(SelectOptionBulkEditAssets),
      name: z.string(),
...
export const BulkEditDialog: React.FC<BulkEditAssetsDialogProps> = ({ selectedAssets, setSelectedAssets }) => {
  const [open, setOpen] = useState(false)
  const { mutateAsync: bulkEdit } = useBulkEditAsset()
  ...
  const allOptionSelects = useMemo(() => {
    return getAllSelectOptionsForBulkEditAssets()
  }, [])
  ...
  const onSubmit = async () => {
    const ids = selectedAssets.map((object) => object.id)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <FormProvider {...form}>
        <DialogTrigger asChild>
          <Button disabled={selectedAssets.length === 0} icon={<Pencil />} iconPosition="left" variant="secondary">
            {selectedAssets && selectedAssets.length > 0 ? `Bulk Edit (${selectedAssets.length})` : 'Bulk Edit'}
          </Button>
        </DialogTrigger>
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogContent onOpenAutoFocus={(e) => e.preventDefault()} className="max-w-[580px]">
            ...
            <div className="flex flex-col gap-4 mt-4">
                  ...
                  <div key={item.id} className="flex items-center gap-2 w-full">
                    <div className="flex flex-col items-start gap-2">
                      <Select
                        ...
                        onValueChange={(value) => {
                          const selectedEnum = value as SelectOptionBulkEditAssets
                          update(index, {
                            value: selectedEnum,
                      ...
                      >
                        ...
                        <SelectContent>
                          {Object.values(SelectOptionBulkEditAssets).map((option) => (
              ...
              {fields.length < Object.keys(SelectOptionBulkEditAssets).length ? (
                <Button
                  icon={<Plus />}
                  onClick={() =>
                    append({

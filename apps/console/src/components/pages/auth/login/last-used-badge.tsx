...
export const LastUsedBadge = ({ provider, lastUsedProvider, floating = false, className }: LastUsedBadgeProps) => {
  ...
  return (
    <Badge variant="primary" className={cn('bg-primary/80 text-white', floating ? 'absolute -top-3.5 left-1/2 -translate-x-1/2 z-10 w-fit whitespace-nowrap' : 'self-center', className)}>
      Last used
    </Badge>
  )
}

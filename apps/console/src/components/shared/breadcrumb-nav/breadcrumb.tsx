...
export const BreadcrumbNavigation = ({ homeElement = 'Home' }: TBreadCrumbProps) => {
  ...
  if (pathname.startsWith('/programs')) {
    ...
    if (pathname.includes('/control-implementation')) {
      breadcrumbs.push({
        name: 'Control Implementations',
      })
    }
    if (pathname.includes('/create-control')) {
      breadcrumbs.push({
        name: 'Create Control',
      })
    }
    if (pathname.includes('/create-subcontrol')) {
      breadcrumbs.push({
        name: 'Create Subcontrol',
      })
    }
    if (pathname.includes('/map-controls')) {
      breadcrumbs.push({
        name: 'Map Controls',
      })
    }

...
const ExistingFilesTable: React.FC<TProps> = ({ tableKey, selectedFileIds, onSelect, onDeselect, where, defaultCategory }) => {
  ...
  const scopedWhere = useMemo<FileWhereInput>(() => ({ ...where, ...(category === ALL_CATEGORIES ? {} : { categoryName: category }) }), [where, category])

  const { searchTerm, search, resetPagination, rows, isFetching, pagination, setPagination, paginationMeta, previewFile, setPreviewFile } = useFilesTableState({ tableKey, where: scopedWhere })

  const selectedIds = useMemo(() => new Set(selectedFileIds), [selectedFileIds])

  const toSelection = (file: TFileRow): TExistingFileSelection => ({
    type: 'existingFile',
    id: file.id,
  ...
  }, [rows, selectedIds, onSelect, onDeselect, setPreviewFile])

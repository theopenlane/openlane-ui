...
const ObjectAssociationGraph: React.FC<TObjectAssociationGraphProps> = ({ centerNode, sections, isFullscreen, closeFullScreen, controlId, onGroupSelect, clearGroupRef }) => {
      ...
      if ('edges' in connection && Array.isArray(connection.edges)) {
        ...
        groups[sectionType] = {
          ...
          items: nodes.map((node) => ({
            ...node,
            name: node.name || node.fullName || node.title || '',
            refCode: node.refCode || node.name || node.fullName || node.title || '',
  ...
  const { graphData, colorMap, centerNodeMeta } = useMemo(() => {
    ...
    if (!colorMap[centerNode.type]) {
      const centerMapEntry = ObjectAssociationMap[centerNode.type as keyof typeof ObjectAssociationMap]
      colorMap[centerNode.type] = centerMapEntry ? resolveCssVar(centerMapEntry.color) : '#ccc'
    }

    Object.entries(groupedSections).forEach(([sectionType, group]) => {
      if (!colorMap[sectionType]) {
        const sectionMapEntry = ObjectAssociationMap[sectionType as keyof typeof ObjectAssociationMap]
        colorMap[sectionType] = sectionMapEntry ? resolveCssVar(sectionMapEntry.color) : '#ccc'
      }
      nodes.push({
        id: sectionType,
        name: group.label,

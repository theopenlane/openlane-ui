...
const variantToVars: Record<Variant, { bg: string; border: string; icon: string; iconEl: LucideIcon; link: string; text?: string }> = {
  ...
  success: {
    ...
    iconEl: CheckCircle2,
    link: '[&_a]:text-foreground [&_a]:decoration-[var(--color-success)]',
  },
  warning: {
    ...
    link: '[&_a]:text-foreground [&_a]:decoration-[var(--color-warning)]',
  },
  suggestion: {
    ...
    link: '[&_a]:text-foreground [&_a]:decoration-[var(--color-suggestion)]',
  },
  danger: {
    ...
    iconEl: AlertCircle,
    link: '[&_a]:text-foreground [&_a]:decoration-[var(--color-danger)]',
  },
  simple: {
    bg: '',
  ...
  recommendation: {
    ...
    iconEl: Sparkles,
    link: '[&_a]:text-foreground [&_a]:decoration-[var(--color-recommendation)]',
  },
}
...
export const Callout = ({ variant = 'info', title, compact, contentClassName, icon, action, className, children, ...props }: CalloutProps) => {
  const v = variantToVars[variant]
  const Icon = icon ?? v.iconEl

  return (
    <div className={cn('rounded-lg border p-4', compact ? 'py-3' : 'pb-5', v.bg, v.border, className)} {...props}>
      <div className="flex gap-3">
        <div className={cn('mt-[2px] shrink-0', v.icon)}>

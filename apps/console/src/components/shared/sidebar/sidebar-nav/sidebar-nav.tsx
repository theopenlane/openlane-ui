...
export default function SideNav({
  ...
  const SidebarChildLink: React.FC<{ child: NavItem }> = ({ child }) => {
    ...
    const linkContent = (
      <Link
        href={child.href ?? '#'}
        className={`flex items-center gap-2 px-2.5 mb-2 h-8 rounded-md hover:bg-card text-muted-foreground transition-colors duration-500 ${isActive ? 'bg-card text-paragraph' : ''}`}
      >

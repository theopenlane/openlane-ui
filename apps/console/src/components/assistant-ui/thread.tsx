...
export interface ThreadProps {
  welcome?: {
    message?: string
    suggestions?: ThreadWelcomeSuggestion[]
  }
}
...
const ThreadWelcome = ({ message, suggestions }: ThreadProps['welcome'] = {}) => {
  return (
    <div className="aui-thread-welcome-root mx-auto my-auto flex w-full max-w-(--thread-max-width) grow flex-col">
      <div className="aui-thread-welcome-center flex w-full grow flex-col items-center justify-center">
        <div className="aui-thread-welcome-message flex size-full flex-col justify-center px-4">
          <h1 className="aui-thread-welcome-message-inner fade-in slide-in-from-bottom-1 animate-in font-semibold text-2xl duration-200">Hello there!</h1>
          <p className="aui-thread-welcome-message-inner fade-in slide-in-from-bottom-1 animate-in text-muted-foreground text-xl delay-75 duration-200">{message || 'How can I help you today?'}</p>
        </div>
      </div>
...
export type ThreadWelcomeSuggestion = {
  text: React.ReactNode
  prompt: string
  label?: React.ReactNode
}

const ThreadSuggestions: FC<{ suggestions?: ThreadWelcomeSuggestion[] }> = ({ suggestions }) => {
  ...
  return (
    <div className="aui-thread-welcome-suggestions grid w-full @md:grid-cols-2 gap-2 pb-4">
      {suggestions.map((suggestion, index) => (
        ...
        >
          <ThreadPrimitive.Suggestion prompt={suggestion.prompt} asChild>
            <Button
              variant="ghost"
              className="aui-thread-welcome-suggestion h-auto w-full @md:flex-col flex-wrap items-start justify-start gap-1 rounded-2xl border px-4 py-3 text-left text-sm transition-colors hover:bg-muted"
            >
              <span className="aui-thread-welcome-suggestion-text-1 font-medium w-full">{suggestion.text}</span>

              {'label' in suggestion && <span className="aui-thread-welcome-suggestion-text-2 text-muted-foreground">{suggestion.label}</span>}
            </Button>
...
const BranchPicker: FC<BranchPickerPrimitive.Root.Props> = ({ className, ...rest }) => {
  return (
    <BranchPickerPrimitive.Root hideWhenSingleBranch className={cn('aui-branch-picker-root mr-2 -ml-2 inline-flex items-center text-muted-foreground text-xs', className)} {...rest}>

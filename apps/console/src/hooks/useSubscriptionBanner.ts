...
export function useSubscriptionBanner() {
  ...
  const { expiresAt, trialExpiresAt, stripeSubscriptionStatus: stripeStatus } = subscription || {}
  ...
  const bannerText = (() => {
    ...
    // paymentMethodAdded has been removed from backend and GET_ORGANIZATION_BILLING_BANNER query.

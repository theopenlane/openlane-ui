...
export function useHasScrollbar(deps: unknown[] = []) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps, @eslint-react/exhaustive-deps -- deps are forwarded from callers to re-run the scroll check when their data changes
  }, deps)

...
import {
  ...
  CreateBulkCsvScheduledJobMutation,
  CreateBulkCsvTaskMutationVariables,
  DeleteScheduledJobMutation,
  DeleteScheduledJobMutationVariables,
  DeleteBulkScheduledJobMutation,
  DeleteBulkScheduledJobMutationVariables,
  UpdateScheduledJobMutation,
  UpdateScheduledJobMutationVariables,
  UpdateBulkScheduledJobMutation,
  UpdateBulkScheduledJobMutationVariables,
...
import {
  ...
  BULK_DELETE_SCHEDULED_JOB,
  CREATE_SCHEDULED_JOB,
  CREATE_CSV_BULK_SCHEDULED_JOB,
  DELETE_SCHEDULED_JOB,
  UPDATE_SCHEDULED_JOB,
  BULK_EDIT_SCHEDULED_JOB,
} from '@repo/codegen/query/scheduled-job'
...
export const useCreateBulkCSVScheduledJob = () => {
  const { queryClient } = useGraphQLClient()

  return useMutation<CreateBulkCsvScheduledJobMutation, unknown, CreateBulkCsvTaskMutationVariables>({
    mutationFn: async (variables) => fetchGraphQLWithUpload({ query: CREATE_CSV_BULK_SCHEDULED_JOB, variables }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['scheduledJobs'] })
    },
  })
...
export const useBulkEditScheduledJob = () => {
  const { client, queryClient } = useGraphQLClient()
  return useMutation<UpdateBulkScheduledJobMutation, unknown, UpdateBulkScheduledJobMutationVariables>({
    mutationFn: async (variables) => client.request(BULK_EDIT_SCHEDULED_JOB, variables),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['scheduledJobs'] })
    },
  })
...
export const useBulkDeleteScheduledJob = () => {
  const { client, queryClient } = useGraphQLClient()

  return useMutation<DeleteBulkScheduledJobMutation, unknown, DeleteBulkScheduledJobMutationVariables>({
    mutationFn: async (variables) => client.request(BULK_DELETE_SCHEDULED_JOB, variables),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['scheduledJobs'] })
    },
  })

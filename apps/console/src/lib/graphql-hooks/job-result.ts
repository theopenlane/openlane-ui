...
import {
  ...
  CreateBulkCsvJobResultMutation,
  CreateBulkCsvTaskMutationVariables,
  DeleteJobResultMutation,
  DeleteJobResultMutationVariables,
  DeleteBulkJobResultMutation,
  DeleteBulkJobResultMutationVariables,
  UpdateJobResultMutation,
  UpdateJobResultMutationVariables,
  UpdateBulkJobResultMutation,
  UpdateBulkJobResultMutationVariables,
...
import {
  JOB_RESULT,
  GET_ALL_JOB_RESULTS,
  BULK_DELETE_JOB_RESULT,
  CREATE_JOB_RESULT,
  CREATE_CSV_BULK_JOB_RESULT,
  DELETE_JOB_RESULT,
  UPDATE_JOB_RESULT,
  BULK_EDIT_JOB_RESULT,
} from '@repo/codegen/query/job-result'
...
export const useCreateBulkCSVJobResult = () => {
  const { queryClient } = useGraphQLClient()

  return useMutation<CreateBulkCsvJobResultMutation, unknown, CreateBulkCsvTaskMutationVariables>({
    mutationFn: async (variables) => fetchGraphQLWithUpload({ query: CREATE_CSV_BULK_JOB_RESULT, variables }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['jobResults'] })
    },
  })
...
export const useBulkEditJobResult = () => {
  const { client, queryClient } = useGraphQLClient()
  return useMutation<UpdateBulkJobResultMutation, unknown, UpdateBulkJobResultMutationVariables>({
    mutationFn: async (variables) => client.request(BULK_EDIT_JOB_RESULT, variables),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['jobResults'] })
    },
  })
...
export const useBulkDeleteJobResult = () => {
  const { client, queryClient } = useGraphQLClient()

  return useMutation<DeleteBulkJobResultMutation, unknown, DeleteBulkJobResultMutationVariables>({
    mutationFn: async (variables) => client.request(BULK_DELETE_JOB_RESULT, variables),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['jobResults'] })
    },
  })

...
import { ASSET, ASSETS_WITH_FILTER, BULK_DELETE_ASSET, CREATE_ASSET, CREATE_CSV_BULK_ASSET, DELETE_ASSET, UPDATE_ASSET } from '@repo/codegen/query/asset'
...
export const useAssetsWithFilter = ({ where, orderBy, pagination, enabled = true }: GetAllAssetsArgs) => {
  ...
  const queryResult = useQuery<AssetsWithFilterQuery, unknown>({
    ...
    queryFn: async (): Promise<AssetsWithFilterQuery> => {
      const result = await client.request(ASSETS_WITH_FILTER, { where, orderBy, ...pagination?.query })
...
// TODO: add bulk update to backend

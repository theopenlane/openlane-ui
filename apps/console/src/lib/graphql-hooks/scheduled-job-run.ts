...
import {
  ...
  CreateBulkCsvScheduledJobRunMutation,
  CreateBulkCsvTaskMutationVariables,
  DeleteScheduledJobRunMutation,
  DeleteScheduledJobRunMutationVariables,
  DeleteBulkScheduledJobRunMutation,
  DeleteBulkScheduledJobRunMutationVariables,
  UpdateScheduledJobRunMutation,
  UpdateScheduledJobRunMutationVariables,
  UpdateBulkScheduledJobRunMutation,
  UpdateBulkScheduledJobRunMutationVariables,
} from '@repo/codegen/src/schema'
import { fetchGraphQLWithUpload } from '@/lib/fetchGraphql'
import { TPagination } from '@repo/ui/pagination-types'
import {
  SCHEDULED_JOB_RUN,
  GET_ALL_SCHEDULED_JOB_RUNS,
  BULK_DELETE_SCHEDULED_JOB_RUN,
  CREATE_SCHEDULED_JOB_RUN,
  CREATE_CSV_BULK_SCHEDULED_JOB_RUN,
  DELETE_SCHEDULED_JOB_RUN,
  UPDATE_SCHEDULED_JOB_RUN,
  BULK_EDIT_SCHEDULED_JOB_RUN,
} from '@repo/codegen/query/scheduled-job-run'
...
export const useCreateBulkCSVScheduledJobRun = () => {
  const { queryClient } = useGraphQLClient()

  return useMutation<CreateBulkCsvScheduledJobRunMutation, unknown, CreateBulkCsvTaskMutationVariables>({
    mutationFn: async (variables) => fetchGraphQLWithUpload({ query: CREATE_CSV_BULK_SCHEDULED_JOB_RUN, variables }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['scheduledJobRuns'] })
    },
  })
...
export const useBulkEditScheduledJobRun = () => {
  const { client, queryClient } = useGraphQLClient()
  return useMutation<UpdateBulkScheduledJobRunMutation, unknown, UpdateBulkScheduledJobRunMutationVariables>({
    mutationFn: async (variables) => client.request(BULK_EDIT_SCHEDULED_JOB_RUN, variables),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['scheduledJobRuns'] })
    },
  })
...
export const useBulkDeleteScheduledJobRun = () => {
  const { client, queryClient } = useGraphQLClient()

  return useMutation<DeleteBulkScheduledJobRunMutation, unknown, DeleteBulkScheduledJobRunMutationVariables>({
    mutationFn: async (variables) => client.request(BULK_DELETE_SCHEDULED_JOB_RUN, variables),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['scheduledJobRuns'] })
    },
  })

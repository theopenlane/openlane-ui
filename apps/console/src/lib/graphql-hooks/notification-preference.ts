...
export type NotificationPreferencesNode = NonNullable<NonNullable<NonNullable<NotificationPreferencesWithFilterQuery['notificationPreferences']>['edges']>[number]>['node']

export type NotificationPreferencesNodeNonNull = NonNullable<NotificationPreferencesNode>
...
export const useNotificationPreferencesWithFilter = ({ where, orderBy, pagination, enabled = true }: GetAllNotificationPreferencesArgs) => {
  ...
  const queryResult = useQuery<NotificationPreferencesWithFilterQuery, unknown>({
    ...
    queryFn: async (): Promise<NotificationPreferencesWithFilterQuery> => {
      const result = await client.request<NotificationPreferencesWithFilterQuery>(GET_ALL_NOTIFICATION_PREFERENCES, { where, orderBy, ...pagination?.query })
      return result
    },
    enabled,
  })

  const edges = queryResult.data?.notificationPreferences?.edges ?? []

  const notificationPreferencesNodes: NotificationPreferencesNodeNonNull[] = edges.filter((edge) => edge != null).map((edge) => edge?.node as NotificationPreferencesNodeNonNull)

  return { ...queryResult, notificationPreferencesNodes }
}

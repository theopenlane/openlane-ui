 ...
 */
export const buildStructuredPrompt = ({ backgroundData, examples, history, outputFormatting, prefilledResponse, question, rules, task, taskContext, thinking, tone }: StructuredPromptSections) => {
  const formattedExamples = Array.isArray(examples) ? examples.map((example) => tag('example', example)).join('\n') : examples

  ...
  return (
    <>
      ...
      <span className="relative min-h-[1lh]">
        ...
        <Combobox ref={ref} className={cn('absolute top-0 left-0 size-full bg-transparent outline-hidden', className)} value={value} autoSelect {...inputProps} {...props} />
      </span>
    </>
  )
})
...
function InlineComboboxGroup({ className, ...props }: React.ComponentProps<typeof ComboboxGroup>) {
  return <ComboboxGroup {...props} className={cn('hidden py-1.5 not-last:border-b [&:has([role=option])]:block', className)} />
}

function InlineComboboxGroupLabel({ className, ...props }: React.ComponentProps<typeof ComboboxGroupLabel>) {
  return <ComboboxGroupLabel {...props} className={cn('mt-1.5 mb-2 px-3 text-xs font-medium text-muted-foreground', className)} />
}

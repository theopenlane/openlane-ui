...
import { PlateElement, useEditorRef, useElement, usePluginOption, useReadOnly, useSelected } from '@udecode/plate/react'
...
export const TableRowElement = withRef<typeof PlateElement>(({ children, className, ...props }, ref) => {
  ...
  return (
    <PlateElement ref={useComposedRef(ref, previewRef)} as="tr" className={cn(className, 'group/row', isDragging && 'opacity-50')} data-selected={selected ? 'true' : undefined} {...props}>
      {hasControls && (

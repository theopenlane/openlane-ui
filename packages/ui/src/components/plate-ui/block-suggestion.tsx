...
export const BlockSuggestionCard = ({ idx, isLast, suggestion }: { idx: number; isLast: boolean; suggestion: ResolvedSuggestion }) => {
    ...
    suggestionIds.forEach((id) => {
      ...
      const entries = [
        ...editor.api.nodes<TElement | TSuggestionText>({
          at: [],
          mode: 'all',
          match: (n) => (n[SuggestionPlugin.key] && n[getSuggestionKey(id)]) || api.suggestion.nodeId(n as TElement) === id,
        }),
      ]
...
export function BlockSuggestion({ element }: { element: TSuggestionElement }) {
  ...
  return <div className={cn('pointer-events-none absolute inset-0 z-1 border-2 border-brand/[0.8] transition-opacity', isRemove && 'border-gray-300')} contentEditable={false} />
}

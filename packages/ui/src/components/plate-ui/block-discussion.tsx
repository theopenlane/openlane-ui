...
import { type NodeEntry, type Path, type TElement, createZustandStore, PathApi, TextApi } from '@udecode/plate'
...
export const BlockDiscussion: RenderNodeWrapper<CommentsConfig> = (props) => {
  ...
  return (props) => <BlockCommentsContent blockPath={blockPath} commentNodes={commentNodes} draftCommentNode={draftCommentNode} suggestionNodes={suggestionNodes} {...props} />
}
...
const BlockCommentsContent = ({
  ...
  const selected = resolvedDiscussions.some((d) => d.id === activeCommentId) || resolvedSuggestion.some((s) => s.suggestionId === activeSuggestionId)
  ...
  const anchorElement = useMemo(() => {
    ...
    if (activeCommentId) {
      if (activeCommentId === getDraftCommentKey()) {
        ...
        activeNode = commentNodes.find(([node]) => editor.getApi(CommentsPlugin).comment.nodeId(node) === activeCommentId)
      }
    }

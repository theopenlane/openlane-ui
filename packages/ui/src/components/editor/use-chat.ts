...
const fakeStreamText = ({ chunkCount = 10, editor, sample = null, signal }: { editor: PlateEditor; chunkCount?: number; sample?: 'comment' | 'markdown' | 'mdx' | null; signal?: AbortSignal }) => {
  ...
  return new ReadableStream({
    async start(controller) {
      ...
      if (sample === 'comment') {
        ...
        controller.enqueue(encoder.encode(`data: {"type":"text-start","id":"${messageId}","providerMetadata":{"openai":{"itemId":"${messageId}"}}}\n\n`))
        await new Promise((resolve) => setTimeout(resolve, 10))
        ...
        for (let i = 0; i < blocks.length; i++) {
          ...
          for (const chunk of block) {
            ...
            controller.enqueue(encoder.encode(`data: {"type":"text-delta","id":"${messageId}","delta":"${escapedText}"}\n\n`))
          }
...
const createCommentChunks = (editor: PlateEditor) => {
  ...
  const blocks = selectedBlocks.length > 0 && (editor.api.isExpanded() || isSelectingSome) ? selectedBlocks : editor.children

...
export const calendarStyles = tv({
  slots: {
    ...
    nav_button: 'h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 [&>button]:bg-transparent',
    ...
    cell:
      'h-9 w-9 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent-secondary/50 ' +
      '[&:has([aria-selected])]:bg-accent-secondary first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 ' +

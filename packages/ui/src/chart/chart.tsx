  ...
  return (
    <li className="inline-flex">
      <button
        ...
        className={cn(
          // base
          'group inline-flex flex-nowrap items-center gap-1.5 whitespace-nowrap rounded-sm px-2 py-1 transition bg-transparent',
      ...
      >
        <span className={cn('h-[3px] w-3.5 shrink-0 rounded-full', getColorClassName(color, 'bg'), activeLegend && activeLegend !== name ? 'opacity-40' : 'opacity-100')} aria-hidden={true} />
        <p
          className={cn(
            // base
...
const ChartLegend = (
  ...
  return (
    <div
      ...
      className={cn('flex items-center', { 'justify-center': legendPosition === 'center' }, { 'justify-start': legendPosition === 'left' }, { 'justify-end': legendPosition === 'right' })}
    >
      <Legend
  ...
  return (
    <div ref={ref} className={cn('h-80 w-full', className)} tremor-id="tremor-raw" {...other}>
      <ResponsiveContainer>
        ...
        >
          {showGridLines ? <CartesianGrid className={cn('stroke-gray-200 stroke-1 dark:stroke-gray-800')} horizontal={true} vertical={false} /> : null}
          <XAxis

...
import React, { type HTMLAttributes, type ReactNode, type RefObject, createContext, forwardRef, startTransition, useCallback, useContext, useEffect, useMemo, useState } from 'react'
...
const InlineComboboxContent: typeof ComboboxPopover = ({ className, ...props }) => {
  ...
  return (
    <Portal>
      <ComboboxPopover className={cn('z-[500] max-h-[288px] w-[300px] overflow-y-auto rounded-md bg-white shadow-md dark:bg-oxford-blue-950', className)} {...props} />
    </Portal>
  )
}

const comboboxItemVariants = cva(
  'relative mx-1 flex h-[28px] select-none items-center rounded-sm px-2 text-sm text-oxford-blue-950 outline-none [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 dark:text-oxford-blue-50',
  {
    defaultVariants: {
      interactive: true,
    },
    variants: {
      interactive: {
        false: '',
        true: 'cursor-pointer transition-colors hover:bg-oxford-blue-100 hover:text-oxford-blue-900 data-[active-item=true]:bg-oxford-blue-100 data-[active-item=true]:text-oxford-blue-900 dark:hover:bg-oxford-blue-800 dark:hover:text-oxford-blue-50 dark:data-[active-item=true]:bg-oxford-blue-800 dark:data-[active-item=true]:text-oxford-blue-50',
      },
    },
  },
)
...
const InlineComboboxItem = ({ className, focusEditor = true, group, keywords, label, onClick, ...props }: InlineComboboxItemProps) => {
  ...
  const visible = useMemo(() => !filter || filter({ group, keywords, label, value }, search as string), [filter, group, keywords, label, value, search])
...
const InlineComboboxGroupLabel = withCn(ComboboxGroupLabel, 'mb-2 mt-1.5 px-3 text-xs font-medium text-oxford-blue-500 dark:text-oxford-blue-400')

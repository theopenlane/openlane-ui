...
export const aiSystemInstruction = process.env.AI_SYSTEM_INSTRUCTION?.trim() || 'You are a helpful assistant that provides concise and accurate answers based on the provided context.'
export const controlSystemInstruction =
  process.env.CONTROL_FRAMEWORK_INSTRUCTION?.trim() || 'When referencing controls, always use their ref codes with their framework (e.g., "SOC 2 - CC1.1"). Do not use internal identifiers.'
export const policyPrompt = (policyName: string) => process.env.AI_POLICY_PROMPT?.trim() || `Generate a formal policy document for "${policyName} based on the provided policy name and requirements.`
